...
**Zava** is an AI-powered travel agent built as a containerized FastAPI service. It uses a custom LangGraph workflow with `goto` replan path, **LLM-driven tool-calling** (`bind_tools`), **MCP (Model Context Protocol) tool integration** for destination research, an offline travel knowledge retriever, GPT-4.1 via Azure OpenAI, and full OpenTelemetry GenAI tracing to Application Insights via `configure_azure_monitor()`.
...
3. **Researching the destination via MCP tools** (travel advisory, local phrases)
4. Drafting an initial travel plan via LLM (enriched with MCP research data)
5. Using LLM tool-calling (`bind_tools`) to search flights, hotels, weather, and estimate costs
6. Evaluating budget constraints and replanning if needed (LangGraph `goto`)
7. Producing a final, friendly travel plan summary with visa info and local phrases
...
The `research_destination` node calls a local **FastMCP server** (`app/mcp_server.py`) via stdio transport:

| MCP Tool | Description |
|----------|-------------|
| `get_travel_advisory` | Visa requirements, safety level, currency, timezone, emergency numbers, transport tips |
| `get_local_phrases` | Essential travel phrases in the local language (hello, thank you, help, etc.) |

The MCP server runs as a subprocess — no external API keys needed. Results flow into `draft_plan` and `finalize` to enrich the travel plan with practical and cultural information.

**Simulate MCP failure** for observability demos:
```bash
export MCP_SIMULATE_FAILURE=true   # get_local_phrases will raise ConnectionError
```

### LLM Tool-Calling
The `run_tools` node uses `llm.bind_tools(ALL_TOOLS)` which includes both deterministic tools and MCP-backed tools. The LLM receives tool schemas, decides which to call, and results are fed back. This produces `gen_ai.tool.definitions` on chat spans in App Insights.
...
| `app/graph.py` | LangGraph workflow with `bind_tools()`, `goto` routing, and MCP research node |
| `app/tools.py` | `@tool` decorated tools for LLM tool-calling + MCP tool loading |
| `app/mcp_server.py` | FastMCP server with `get_travel_advisory` and `get_local_phrases` tools |
| `app/mcp_tools.py` | MCP client bridge — spawns subprocess, wraps results as LangChain StructuredTool |
...
| `invoke_agent {node}` | Per-node spans (`research_destination`, `draft_plan`, `run_tools`, etc.) |
| `invoke_agent research_destination` | `app.mcp.*` attributes, `mcp_tool_failure` events on error |
...
| `execute_tool get_travel_advisory` | MCP: visa, safety, currency info |
| `execute_tool get_local_phrases` | MCP: local language phrases |
...
| `OTEL_RECORD_CONTENT` | | Set `true` to record LLM/tool content in traces (default: `true`) |
| `AZURE_TRACING_GEN_AI_CONTENT_RECORDING_ENABLED` | | Set `true` to prevent `[redacted]` in tool arguments/results |
| `OTEL_TRACES_SAMPLER` | | Use `always_on` for demos |
| `OTEL_TRACES_SAMPLER_ARG` | | Sampling ratio (e.g., `1.0` for 100%, `0.1` for 10%) |
| `DEMO_FORCE_GOTO` | | Set `true` to force replan path |
| `MCP_SIMULATE_FAILURE` | | Set `true` to simulate MCP tool failure for observability demos |
...
> **Tip:** To see full tool arguments and LLM message content in App Insights traces,
> both `OTEL_RECORD_CONTENT=true` and `AZURE_TRACING_GEN_AI_CONTENT_RECORDING_ENABLED=true`
> must be set. Without them, content shows as `[redacted]`.
